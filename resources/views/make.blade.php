...
<body>
    ...
    <script>
        ...
        fetch(`http://localhost:8000/input?date=${date.toString()}`, {
            "method": "GET",
            "mode": "cors",
            "credentials": "include"
            // });
        ...
        function changeData() {
            ...
            for (let i = 0; i < window.data.countAvailableTime; i++) {
                let tempRadioButton = createRadioElement("time", window.data.resetTimeName[i], window.data.resetAvailableTime[i]);
        ...
        function createRadioElement(name, displayValue, sendValue) {
            let container = document.createElement("div");
            let text = document.createElement("p");
            text.style.margin = "0 8px"
            text.innerText = displayValue;
            text.style.display = "inline-block"
            container.appendChild(createRadioButton(name, sendValue));
        ...
        function createSubmitButton() {
            ...
            element.setAttribute("value", "Appoint");
            return element;
        }
    ...
    $time = Date('H:i');
    ?>
    <h3 id="makeTxt">Please introduce the required informations in order to create your appointment:</h3>
    <br><br>
    ...
    <script>
        function userDataInput() {
            ...
            var userSubmit = document.getElementById('submit').value;
        ...
        function userTimeInput() {
            var userTime = document.getElementById('time').value
            var userSubmit = document.getElementById('submit').value;

            if (userTime < 900) {
                document.querySelector('#submit').disabled = true;
                alert("We are sorry but at that moment we are unavailable, please select another time, our schedule is between 09:00-13:00 and 15:30-21:00, Thank you!");
            } else if (userTime > 1300 && time < 1530) {
                document.querySelector('#submit').disabled = true;
                alert("We are sorry but at that moment we are unavailable, please select another time, our schedule is between 09:00-13:00 and 15:30-21:00, Thank you!");
